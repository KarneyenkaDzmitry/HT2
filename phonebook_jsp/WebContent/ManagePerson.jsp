        ...
        <td><table>
         <%
          for(Map.Entry<String, String> pair : person.getPhones().entrySet())
           { 
         	%><tr>
         	<td align="left" width="40%"><%out.write(pair.getValue() + "\n");%></td>
         	<td  align = "center" width="30%">
         	<input type="hidden" name="phone_id" value="<%=pair.getKey()%>"/>
         	<a href="<%=request.getContextPath()%>/?action=edit_phone&id=<%=person.getId()%>&phone_id=<%=pair.getKey()%>">Редактировать</a>
         	</td>
         	<td  align = "center" width="30%"><a href="<%=request.getContextPath()%>/?action=delete_phone&id=<%=person.getId()%>&phone_id=<%=pair.getKey()%>">Удалить</a></td></tr><%
           }
         %></table>
