...
public class ManagePersonServlet extends HttpServlet {
    ...
    private String validateNumber(String number) {
        ...
        if (!Person.validateNumber(number)) {
            error_message += "Телефон должен быть строкой от 2 до 50 символов из цифр, знаков: плюс, минус и #.<br />";
        }
        return error_message;
    }
        ...
        // Если же действие указано, то...
            ...
            switch (action) {
                ...
                case "add_phone":
                    ...
                    // Извлечение из телефонной книги информации о редактируемой записи.
                    Person editable_person_phones = this.phonebook.getPerson(id);
                    ...
                    // Установка параметров JSP.
                    request.setAttribute("person", editable_person_phones);
                    request.setAttribute("jsp_parameters", jsp_parameters);
                ...
                case "edit_phone":
                    ...
                    // Извлечение из телефонной книги информации о редактируемой записи.
                    Person editable_phone = this.phonebook.getPerson(id);
                ...
                case "delete_phone":
                    ...
                    Person updated_person = this.phonebook.getPerson(id);
                    // Если запись удалось удалить...
                    if (phonebook.deleteNumber(id, phone_id)) {
                        jsp_parameters.put("current_action_result", "DELETION_SUCCESS");
                        jsp_parameters.put("current_action_result_label", "Удаление выполнено успешно");
                    }
                    // Если запись не удалось удалить (например, такой записи нет)...
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Действия (add_go, edit_go,add_phone_go, edit_phone_go) и идентификатор записи (id) над которой выполняется это действие.
        ...
        if (add_phone_go != null) {
            //Получение введённого пользователем номера
            ...
            if (error_message.equals("")) {
                ...
                // Если запись удалось добавить...
                if (this.phonebook.addNumber(editable_person, number)) {
        ...
        if (edit_phone_go != null) {
            //Получение номера
            String number = request.getParameter("number");
            //Получение id - редактируемого телефона
            String phone_id = request.getParameter("phone_id");
            ...
            if (error_message.equals("")) {
                ...
                if (this.phonebook.updateNumber(id, phone_id, number)) {
                    jsp_parameters.put("current_action_result", "UPDATE_SUCCESS");
                    jsp_parameters.put("current_action_result_label", "Обновление выполнено успешно");
                }
